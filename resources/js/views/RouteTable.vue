		...
		<table class="table w-full" cellpadding="0" cellspacing="0">
			...
			>
				<td class="whitespace-no-wrap text-left">
					<div>
						<tooltip :value="route.action">
							{{ route.host }}/{{ route.uri }}
						</tooltip>
					</div>
					<div class="font-70 text-xs">{{ route.as }}</div>
				</td>
				<td class="whitespace-no-wrap text-left">
					<span v-for="value in route.methods"
						  :class="{
							'px-2 py-1 text-xs font-semibold mr-2': ['GET', 'HEAD', 'POST', 'PUT', 'PATCH', 'DELETE'].includes(value),
